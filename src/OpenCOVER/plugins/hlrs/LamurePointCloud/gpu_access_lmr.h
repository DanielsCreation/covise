...
#ifndef CO_LAMURE_REN_GPU_ACCESS_H
#define CO_LAMURE_REN_GPU_ACCESS_H
...
class gpu_access_lmr
{
  public:
    gpu_access_lmr(lmr_device device, const slot_t num_slots, const uint32_t num_surfels_per_node, bool create_layout = true);
    gpu_access_lmr(lmr_device device, const slot_t num_slots, const uint32_t num_surfels_per_node, Data_Provenance_lmr const& data_provenance, bool create_layout = true);
    ~gpu_access_lmr();
    ...
    char *map(lmr_device const &device);
    void unmap(lmr_device const &device);
    char *map_provenance(lmr_device const &device);
    void unmap_provenance(lmr_device const &device);
    ...
    static const size_t query_video_memory_in_mb(lmr_device const& device);
