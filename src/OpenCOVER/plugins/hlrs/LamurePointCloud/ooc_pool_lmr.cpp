...
ooc_pool_lmr::ooc_pool_lmr(const uint32_t num_threads, const size_t size_of_slot_in_bytes) : locked_(false), size_of_slot_(size_of_slot_in_bytes), num_threads_(num_threads), shutdown_(false), bytes_loaded_(0)
{
    assert(num_threads_ > 0);

    // configure semaphore_lmr
    ...
    {
        threads_.push_back(std::thread(&ooc_pool_lmr::run, this));
    }
}

ooc_pool_lmr::ooc_pool_lmr(const uint32_t num_threads, const size_t size_of_slot_in_bytes, const size_t size_of_slot_provenance, Data_Provenance_lmr const &data_provenance)
...
bool ooc_pool_lmr::is_shutdown()
{
    std::lock_guard<std::mutex> lock(mutex_);
    return shutdown_;
}

void ooc_pool_lmr::lock()
{
    mutex_.lock();
    locked_ = true;
}

void ooc_pool_lmr::unlock()
{
    locked_ = false;
    mutex_.unlock();
}

void ooc_pool_lmr::begin_measure()
{
    std::lock_guard<std::mutex> lock(mutex_);
    bytes_loaded_ = 0;
}

void ooc_pool_lmr::end_measure()
{
    std::lock_guard<std::mutex> lock(mutex_);
    std::cout << "megabytes loaded: " << bytes_loaded_ / 1024 / 1024 << std::endl;
}

void ooc_pool_lmr::run()
{
    model_database_lmr *database = model_database_lmr::get_instance();
    ...
    while(true)
    {
        ...
        cache_queue_lmr::job job = priority_queue_.top_job();
        ...
        if(job.node_id_ != invalid_node_t)
        {
            ...
            if(_data_provenance.get_size_in_bytes() > 0) {
                provenance_stream_lmr access_provenance;
...
void ooc_pool_lmr::resolve_cache_history(cache_index_lmr *index)
{
    assert(locked_);
...
void ooc_pool_lmr::perform_queue_maintenance(cache_index_lmr *index)
{
    assert(locked_);
...
bool ooc_pool_lmr::acknowledge_request(cache_queue_lmr::job job)
{
...
cache_queue_lmr::query_result ooc_pool_lmr::acknowledge_query(const model_t model_id, const node_t node_id) { return priority_queue_.is_node_indexed(model_id, node_id); }

void ooc_pool_lmr::acknowledge_update(const model_t model_id, const node_t node_id, int32_t priority) { priority_queue_.update_job(model_id, node_id, priority); }
