...
cut_database_record_lmr::
cut_database_record_lmr(const context_t context_id)
...
void cut_database_record_lmr::
set_camera(const view_t view_id, const camera_lmr& camera) {
...
void cut_database_record_lmr::
receive_cameras(std::map<view_t, camera_lmr>& cameras) {
    //cameras.clear();
    ...
    if (current_front_ == record_front::FRONT_A) {
        for (const auto& cam_it : front_b_cameras_) {
            camera_lmr camera = cam_it.second;
...
expand_front_a(const view_t view_id, const model_t model_id) {
    ...
    {
        //expand cut_lmr front B
        front_a_cuts_.push_back(std::vector<cut_lmr>());
    ...
    {
        front_a_cuts_[model_id].push_back(cut_lmr(context_id_, new_view_id, model_id));
...
expand_front_b(const view_t view_id, const model_t model_id) {
    ...
    {
        //expand cut_lmr front B
        front_b_cuts_.push_back(std::vector<cut_lmr>());
    ...
    {
        front_b_cuts_[model_id].push_back(cut_lmr(context_id_, new_view_id, model_id));
...
cut_lmr& cut_database_record_lmr::
...
void cut_database_record_lmr::
set_cut(const view_t view_id, const model_t model_id, cut_lmr& cut_lmr) {
    is_swap_required_ = true;

    if (current_front_ == record_front::FRONT_A) {
        expand_front_b(view_id, model_id);
        front_b_cuts_[model_id][view_id] = cut_lmr;
    }
    else {
        expand_front_a(view_id, model_id);
        front_a_cuts_[model_id][view_id] = cut_lmr;
    }

}

std::vector<cut_database_record_lmr::slot_update_desc>& cut_database_record_lmr::
get_updated_set() {
    if (current_front_ == record_front::FRONT_A) {
...
void cut_database_record_lmr::
set_updated_set(std::vector<cut_database_record_lmr::slot_update_desc>& updated_set) {
    is_swap_required_ = true;
...
const bool cut_database_record_lmr::
...
const cut_database_record_lmr::temporary_buffer cut_database_record_lmr::
get_buffer() const {
    if (current_front_ == record_front::FRONT_A) {
...
void cut_database_record_lmr::
set_buffer(const cut_database_record_lmr::temporary_buffer buffer) {
    is_swap_required_ = true;
