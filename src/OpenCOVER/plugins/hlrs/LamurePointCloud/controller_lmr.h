...
#ifndef CO_REN_CONTROLLER_H_
#define CO_REN_CONTROLLER_H_
...
class controller_lmr
{
  public:
    ...
    static controller_lmr *get_instance();

    controller_lmr(const controller_lmr &) = delete;
    controller_lmr &operator=(const controller_lmr &) = delete;
    virtual ~controller_lmr();

    void signal_system_reset();
    void reset_system();
    void reset_system(Data_Provenance_lmr const &data_provenance);
    ...
    void dispatch(const context_t context_id, lmr_device device);
    void dispatch(const context_t context_id, lmr_device device, Data_Provenance_lmr const& data_provenance);

    const bool is_cut_update_in_progress(const context_t context_id);
    const bool is_cut_update_in_progress(const context_t context_id, Data_Provenance_lmr const &data_provenanc);

    scm::gl::buffer_ptr get_context_buffer(const context_t context_id, lmr_device device);
    scm::gl::buffer_ptr get_context_buffer(const context_t context_id, lmr_device device, Data_Provenance_lmr const& data_provenance);
    scm::gl::vertex_array_ptr get_context_memory(const context_t context_id, bvh::primitive_type type, lmr_device device);
    scm::gl::vertex_array_ptr get_context_memory(const context_t context_id, bvh::primitive_type type, lmr_device device, Data_Provenance_lmr const& data_provenance);
  ...
  protected:
    ...
    static controller_lmr *single_;

  private:
    static std::mutex mutex_;

    std::unordered_map<context_t, cut_update_pool_lmr *> cut_update_pools_;
    std::unordered_map<context_t, gpu_context_lmr *> gpu_contexts_;
