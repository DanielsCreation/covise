...
gpu_access_lmr::gpu_access_lmr(lmr_device device, const slot_t num_slots, const uint32_t num_surfels_per_node, bool create_layout)
      ...
      has_layout_(create_layout)
{
    ...
    std::cout << "lamure: gpu-cache_lmr size (MB): " << buffer_->descriptor()._size / 1024 / 1024 << " (WITHOUT_PROVENANCE)" << std::endl;
#endif
}


gpu_access_lmr::gpu_access_lmr(lmr_device device, const slot_t num_slots, const uint32_t num_surfels_per_node, Data_Provenance_lmr const& data_provenance, bool create_layout)
    : num_slots_(num_slots),
    size_of_surfel_(8 * sizeof(float)),
    size_of_surfel_qz_(3 * sizeof(float)),
    is_mapped_(false),
    ...
    has_layout_(create_layout)
{
    ...
    if (has_layout_)
    {
        ...
        std::cout << policy_lmr::get_instance()->size_of_provenance() << std::endl;
...
#ifdef LAMURE_ENABLE_INFO
    std::cout << "lamure: gpu-cache_lmr size (MB): " << buffer_->descriptor()._size / 1024 / 1024 << " (WITH PROVENANCE)" << std::endl;
#endif
}

gpu_access_lmr::~gpu_access_lmr()
{
    buffer_.reset();
...
char *gpu_access_lmr::map(lmr_device const &device)
{
    if (!is_mapped_)
    {
        assert(device);
...
void gpu_access_lmr::unmap(lmr_device const &device)
{
    if (is_mapped_)
    {
        assert(device);
...
char *gpu_access_lmr::map_provenance(lmr_device const &device)
{
...
void gpu_access_lmr::unmap_provenance(lmr_device const &device)
{
    if (is_mapped_provenance_)
    {
        assert(device);
...
scm::gl::vertex_array_ptr gpu_access_lmr::get_memory(bvh::primitive_type type)
{
    switch (type)
    {
...
const size_t gpu_access_lmr::query_video_memory_in_mb(lmr_device const &device)
{
    int size_in_kb;
