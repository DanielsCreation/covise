...
gpu_context_lmr::gpu_context_lmr(const context_t context_id)
...
gpu_context_lmr::~gpu_context_lmr()
{
...
void gpu_context_lmr::create(lmr_device device)
{
    assert(device);
    if (is_created_)
    {
    ...
    temp_buffer_a_ = new gpu_access_lmr(device, upload_budget_in_nodes_, database->get_primitives_per_node(), false);
    temp_buffer_b_ = new gpu_access_lmr(device, upload_budget_in_nodes_, database->get_primitives_per_node(), false);
    primary_buffer_ = new gpu_access_lmr(device, render_budget_in_nodes_, database->get_primitives_per_node(), true);

    map_temporary_storage(cut_database_record_lmr::temporary_buffer::BUFFER_A, device);
    map_temporary_storage(cut_database_record_lmr::temporary_buffer::BUFFER_B, device);
}


void gpu_context_lmr::create(lmr_device device, Data_Provenance_lmr const& data_provenance)
{
    assert(device);
    if (is_created_)
    {
    ...
    primary_buffer_ = new gpu_access_lmr(device, render_budget_in_nodes_, database->get_primitives_per_node(), data_provenance, true);
}

void gpu_context_lmr::test_video_memory(lmr_device device)
{
    model_database_lmr* database = model_database_lmr::get_instance();
    policy_lmr* policy_lmr = policy_lmr::get_instance();

    float safety = 0.75;
    size_t video_ram_free_in_mb = gpu_access_lmr::query_video_memory_in_mb(device) * safety;

    size_t render_budget_in_mb = policy_lmr->render_budget_in_mb();

    if (policy_lmr->out_of_core_budget_in_mb() == 0)
    {
    ...
    {
        std::cout << "##### " << policy_lmr->render_budget_in_mb() << " MB will be used for the render budget #####" << std::endl;
    }
    long node_size_total = database->get_slot_size();
    render_budget_in_nodes_ = (render_budget_in_mb * 1024 * 1024) / node_size_total;

    size_t max_upload_budget_in_mb = policy_lmr->max_upload_budget_in_mb();
...
void gpu_context_lmr::test_video_memory(lmr_device device, Data_Provenance_lmr const& data_provenance)
{
    model_database_lmr* database = model_database_lmr::get_instance();
    policy_lmr* policy_lmr = policy_lmr::get_instance();
...
scm::gl::buffer_ptr gpu_context_lmr::get_context_buffer(lmr_device device)
{
    if (!is_created_)
        create(device);
...
scm::gl::buffer_ptr gpu_context_lmr::get_context_buffer(lmr_device device, Data_Provenance_lmr const& data_provenance)
{
    if (!is_created_)
...
scm::gl::vertex_array_ptr gpu_context_lmr::get_context_memory(bvh::primitive_type type, lmr_device device)
{
    if (!is_created_)
        create(device);
...
scm::gl::vertex_array_ptr gpu_context_lmr::get_context_memory(bvh::primitive_type type, lmr_device device, Data_Provenance_lmr const& data_provenance)
{
    if (!is_created_)
        create(device, data_provenance);
...
void gpu_context_lmr::map_temporary_storage(const cut_database_record_lmr::temporary_buffer& buffer, lmr_device device)
{
    if (!is_created_)
        create(device);
    ...
    switch (buffer)
    {
    case cut_database_record_lmr::temporary_buffer::BUFFER_A:
    ...
    case cut_database_record_lmr::temporary_buffer::BUFFER_B:
...
void gpu_context_lmr::map_temporary_storage(const cut_database_record_lmr::temporary_buffer& buffer, lmr_device device, Data_Provenance_lmr const& data_provenance)
{
    if (!is_created_)
        create(device, data_provenance);
...
void gpu_context_lmr::unmap_temporary_storage(const cut_database_record_lmr::temporary_buffer& buffer, lmr_device device)
{
    if (!is_created_)
        create(device);
...
void gpu_context_lmr::unmap_temporary_storage(const cut_database_record_lmr::temporary_buffer& buffer, lmr_device device, Data_Provenance_lmr const& data_provenance)
{
    if (!is_created_)
        create(device, data_provenance);
...
bool gpu_context_lmr::update_primary_buffer(const cut_database_record_lmr::temporary_buffer& from_buffer, lmr_device device)
{
    if (!is_created_)
        create(device);
    ...
    case cut_database_record_lmr::temporary_buffer::BUFFER_A:
    {
        if (temp_buffer_a_->is_mapped())
        {
            //throw runtime_error("lamure: gpu_context_lmr::Failed to transfer nodes into main memory on context: " + context_id_);
        }
        std::vector<cut_database_record_lmr::slot_update_desc>& transfer_descr_list = cuts->get_updated_set(context_id_);
    ...
    case cut_database_record_lmr::temporary_buffer::BUFFER_B:
    {
        if (temp_buffer_b_->is_mapped())
        {
            throw std::runtime_error("lamure: gpu_context_lmr::Failed to transfer nodes into main memory on context: " + context_id_);
        }
        std::vector<cut_database_record_lmr::slot_update_desc>& transfer_descr_list = cuts->get_updated_set(context_id_);
        if (!transfer_descr_list.empty())
        {
...
bool gpu_context_lmr::update_primary_buffer_fix(const cut_database_record_lmr::temporary_buffer& from_buffer, lmr_device device, Data_Provenance_lmr const &data_provenance)
{
    if(!is_created_)
        create(device, data_provenance);
