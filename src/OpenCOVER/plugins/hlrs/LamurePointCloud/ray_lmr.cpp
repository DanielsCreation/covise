                                       ...
                                       bool is_wysiwyg, ray::intersection &intersection)
{
    ...
    if(!ooc_cache_lmr->is_node_resident_and_aquired(model_id, 0))
    {
        return false;
    }
    ...
    while(!candidates.empty())
    {
        ...
        for(node_t i = 0; i < (node_t)fan_factor; ++i)
        {
            ...
            if(!ooc_cache_lmr->is_node_resident_and_aquired(model_id, node_id))
            {
                continue;
            }
            ...
            for(node_t k = 0; k < fan_factor; ++k)
            {
                ...
                if(!ooc_cache_lmr->is_node_resident_and_aquired(model_id, child_id))
                {
                    all_children_in_memory = false;
                    break;
                }
            ...
            if(intersect_splats)
            {
                ...
                dataset_lmr::serialized_surfel *surfels = (dataset_lmr::serialized_surfel *)ooc_cache_lmr->node_data(model_id, node_id);
                for(unsigned int k = 0; k < num_surfels_per_node; k += valid_surfel_skip)
                {
                    dataset_lmr::serialized_surfel &surfel = surfels[k];
...
const bool ray::intersect_surfel(const dataset_lmr::serialized_surfel &surfel, const scm::math::vec3f &ray_origin, const scm::math::vec3f &ray_direction, float &t)
{
