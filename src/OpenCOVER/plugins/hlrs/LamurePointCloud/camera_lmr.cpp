...
std::mutex camera_lmr::transform_update_mutex_;

camera_lmr::camera_lmr(const view_t view_id, float near_plane, float far_plane_value_, scm::math::mat4f const &view, scm::math::mat4f const &proj, scm::math::mat4f const& init_tb_mat, float distance, bool fast_travel, bool touch_screen_mode)
...
camera_lmr::camera_lmr(const view_t view_id, scm::math::mat4f const &init_tb_mat, float distance, bool fast_travel, bool touch_screen_mode)
...
//camera_lmr* camera_lmr::instance() {
//    return cam_;
//}

camera_lmr::~camera_lmr() {}

void camera_lmr::set_trackball_center_of_rotation(const scm::math::vec3f &cor)
{
...
void camera_lmr::event_callback(uint16_t code, float value)
{
...
scm::gl::frustum::classification_result const camera_lmr::cull_against_frustum(scm::gl::frustum const &frustum, scm::gl::box const &b) const { return frustum.classify(b); }

scm::gl::frustum const camera_lmr::get_frustum_by_model(scm::math::mat4 const &model) const
{
    switch(cam_state_)
    {
...
void camera_lmr::set_projection_matrix(float opening_angle, float aspect_ratio, float znear, float zfar)
{
    scm::math::perspective_matrix(projection_matrix_, opening_angle, aspect_ratio, znear, zfar);

    near_plane_value_ = znear;
    far_plane_value_ = zfar;

    frustum_ = scm::gl::frustum(this->projection_matrix_ * scm::math::mat4f(trackball_.transform()));
}
void camera_lmr::set_view_matrix(scm::math::mat4d const &in_view)
{
    std::cout << cam_state_ << std::endl;
    switch(cam_state_)
    {
    case CAM_STATE_LAMURE:
...
void camera_lmr::update_trackball_mouse_pos(double x, double y)
{
    trackball_init_x_ = x;
    trackball_init_y_ = y;
}

void camera_lmr::update_trackball(double x, double y, int window_width, int window_height, mouse_state const &mouse_state)
{
...
void camera_lmr::write_view_matrix(std::ofstream &matrix_stream)
{
...
float const camera_lmr::transfer_values(float currentValue, float maxValue) const { return std::pow((std::abs(currentValue) / std::abs(maxValue)), 4); }

float const camera_lmr::remap_value(float value, float oldMin, float oldMax, float newMin, float newMax) const
{
...
scm::math::mat4d const camera_lmr::get_high_precision_view_matrix() const
{
...
scm::math::mat4f const camera_lmr::get_projection_matrix() const { return projection_matrix_; }

std::vector<scm::math::vec3d> camera_lmr::get_frustum_corners() const
{
...
void camera_lmr::update_camera(double x, double y, int window_width, int window_height, mouse_state const &mouse_state, bool keys[])
{
    double nx = 2.0 * double(x - (window_width / 2)) / double(window_width);
    double ny = 2.0 * double(window_height - y - (window_height / 2)) / double(window_height);
    double f = dolly_sens_ < 1.0 ? 0.02 : 0.3;
...
scm::math::mat4f const camera_lmr::get_view_matrix() const
{
    switch(cam_state_)
    {
    case CAM_STATE_LAMURE:
...
scm::math::vec3d camera_lmr::get_cam_pos()
{
...
scm::math::mat4f camera_lmr::get_cam_matrix()
{
    scm::math::mat4f cm = scm::math::inverse(scm::math::mat4f(this->trackball_.transform()));
    return cm;
}

void camera_lmr::sync_osg_cam(camera_lmr* cam_, osg::Camera* osg_cam_) {
