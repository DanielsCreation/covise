...
namespace {
        ...
        const std::unordered_map<unsigned,double>& gpuMsByFrame)
    {
        ...
        double sum_cpu_main=0, sum_wait=0, sum_cpu_bp=0, sum_gpu_bp=0, sum_wait_pct=0;
        double sum_gpu_from_tl = 0.0;

        double sum_covC1=0, sum_covD=0; int cnt_cov=0;
        double sum_rpts=0; int cnt_rpts=0;

        // Renderer / Marks (aufsummiert; werden später /n gemittelt)
        ...
        for (const auto& s : stats) {
            // Frametime & Gesamtdauer
            const double ft = (s.frame_duration_ms > 0.0 ? s.frame_duration_ms : s.rendering_traversals_ms);
            if (ft > 0.0) { ft_ms.push_back(ft); syn.total_duration_ms += ft; }
            ...
            if (auto it = gpuMsByFrame.find(s.frame_number); it != gpuMsByFrame.end())
                sum_gpu_from_tl += it->second;

            // Coverage/Effizienz
            if (s.est_density > 0.0f || s.est_coverage > 0.0f) {
                const double D  = std::max(0.0, double(s.est_density));
                const double C1 = (s.est_coverage > 0.0f) ? double(s.est_coverage)
            ...
            if (auto it = gpuMsByFrame.find(s.frame_number); it!=gpuMsByFrame.end()) {
                ...
                if (gms > 0.0 && s.rendered_splats > 0) { sum_rpts += double(s.rendered_splats)/gms; ++cnt_rpts; }
            }

            // Renderer-Estimates (avg)
            t_est_screen_px   += s.est_screen_px;
            t_est_sum_area_px += s.est_sum_area_px;
            t_est_density     += s.est_density;
            t_est_coverage    += s.est_coverage;
            t_est_coverage_px += s.est_coverage_px;
            t_est_overdraw    += s.est_overdraw;
            t_avg_area_pp     += double(s.avg_area_px_per_prim);

            // --- NEU: Heuristik, ob Marks in Sekunden statt Millisekunden vorliegen ---
            ...
            const bool marks_look_like_seconds = (mx > 0.0f && mx < 0.01f); // z.B. 5.0e-05
            auto norm = [&](float v)->double { return marks_look_like_seconds ? (double(v) * 1000.0) : double(v); };

            // Marks aufsummieren (bereits normalisiert)
            m_draw_impl += norm(s.mark_draw_impl_ms);
            m_p1        += norm(s.mark_pass1_ms);
            m_p2        += norm(s.mark_pass2_ms);
            m_p3        += norm(s.mark_pass3_ms);
            m_disp      += norm(s.mark_dispatch_ms);
            m_bind      += norm(s.mark_context_bind_ms);
            m_est       += norm(s.mark_estimates_ms);
            m_sp        += norm(s.mark_singlepass_ms);
        }

        const double n = double(syn.nFrames);

        // Perzentile & Stutter
        syn.p50 = percentile_select(ft_ms, 0.50);
        syn.p95 = percentile_select(ft_ms, 0.95);
        syn.p99 = percentile_select(ft_ms, 0.99);
        syn.jitter_J = std::max(0.0, syn.p99 - syn.p50);
        for (double ft : ft_ms) {
        ...
        // Mittelwerte
        syn.avg_frame_time_ms = (n ? syn.total_duration_ms / n : 0.0);
        syn.avg_cpu_main_ms   = (n ? sum_cpu_main / n : 0.0);
        syn.avg_gpu_time_ms   = (n ? sum_gpu_from_tl / n : 0.0);
        syn.avg_wait_ms       = (n ? sum_wait / n : 0.0);

        syn.fps_avg = inv_ms_to_fps(syn.avg_frame_time_ms);
        syn.fps_p50 = inv_ms_to_fps(syn.p50);
        syn.fps_p95 = inv_ms_to_fps(syn.p95);
        syn.fps_p99 = inv_ms_to_fps(syn.p99);

        syn.prims_per_frame_avg = syn.avg_rpts_points_per_ms * syn.avg_frame_time_ms;
        syn.prims_per_frame_p50 = syn.avg_rpts_points_per_ms * syn.p50;
        syn.prims_per_frame_p95 = syn.avg_rpts_points_per_ms * syn.p95;
        syn.prims_per_frame_p99 = syn.avg_rpts_points_per_ms * syn.p99;


        syn.avg_cpu_busy_pct  = (n ? sum_cpu_bp / n : 0.0);
        syn.avg_gpu_busy_pct  = (n ? sum_gpu_bp / n : 0.0);
        syn.avg_wait_frac_pct = (n ? sum_wait_pct / n : 0.0);

        syn.avg_rpts_points_per_ms = (cnt_rpts ? (sum_rpts / cnt_rpts) : 0.0);
        syn.avg_covC1 = (cnt_cov ? (sum_covC1 / cnt_cov) : 0.0);
        syn.avg_covD  = (cnt_cov ? (sum_covD  / cnt_cov) : 0.0);

        syn.est_screen_px        = (n ? t_est_screen_px        / n : 0.0);
        syn.est_sum_area_px      = (n ? t_est_sum_area_px      / n : 0.0);
        syn.est_density          = (n ? t_est_density          / n : 0.0);
        syn.est_coverage         = (n ? t_est_coverage         / n : 0.0);
        syn.est_coverage_px      = (n ? t_est_coverage_px      / n : 0.0);
        syn.est_overdraw         = (n ? t_est_overdraw         / n : 0.0);
        syn.avg_area_px_per_prim = (n ? t_avg_area_pp          / n : 0.0);

        syn.mark_draw_impl_ms    = (n ? m_draw_impl / n : 0.0);
        syn.mark_pass1_ms        = (n ? m_p1       / n : 0.0);
        syn.mark_pass2_ms        = (n ? m_p2       / n : 0.0);
        syn.mark_pass3_ms        = (n ? m_p3       / n : 0.0);
        syn.mark_dispatch_ms     = (n ? m_disp     / n : 0.0);
        syn.mark_context_bind_ms = (n ? m_bind     / n : 0.0);
        syn.mark_estimates_ms    = (n ? m_est      / n : 0.0);
        syn.mark_singlepass_ms   = (n ? m_sp       / n : 0.0);

        // === NEU: FPS & Primitive/Frame ableiten ===
        auto inv_ms_to_fps = [](double ms)->double { return (ms > 0.0) ? (1000.0 / ms) : 0.0; };
...
void LamureMeasurement::drawIncrement(bool preDraw, const osg::FrameStamp* frameStamp)
{
    ...
    if (frameStamp) {
        ...
        if (cur != m_currFrameNo) {
            m_currFrameNo = cur;
            m_sampleThisFrame = !m_plugin->getSettings().measure_off
                && wantsSampling(m_currFrameNo);
        }
    }
    ...
    if (haveFS && m_sampleThisFrame) {
        ...
        if (collectFrameStats(m_viewer, frameStamp, s, dbg)) {
            if (m_plugin->getSettings().measure_light) {
                ...
                slim.rendered_splats         = s.rendered_splats;
    ...
    const osg::FrameStamp* fs, FrameStats& stats, bool debugPrint)
{
    ...
    std::vector<TimeBlock> blocks; // nur genutzt, wenn m_collectTimeline == true
    ...
    // --- Pose (nur in Full, spart Arbeit/Output)
    if (m_plugin->getSettings().measure_full) {
        cacheStaticGpuInfo();
    ...
    // --- Abgeleitete Proxys & Boundness
    const float ft       = (stats.frame_duration_ms > 0.0f) ? stats.frame_duration_ms : stats.rendering_traversals_ms;
    const float cpu_main = (stats.cpu_update_ms + stats.cpu_cull_ms + stats.cpu_draw_ms + stats.plugin_ms + stats.isect_ms + stats.opencover_ms);
    const float waits    = (stats.sync_time_ms + stats.swap_time_ms + stats.finish_ms);

    stats.cpu_main_ms = cpu_main;
    stats.wait_ms     = waits;

    if (ft > 0.0f) {
        stats.cpu_busy_pct_proxy = std::clamp(100.0f * (cpu_main / ft), 0.0f, 100.0f);
        const float gpu_clip     = std::min(stats.gpu_time_ms, ft);
        stats.gpu_busy_pct_proxy = std::clamp(100.0f * (gpu_clip / ft), 0.0f, 100.0f);
        stats.wait_frac_pct      = std::max(0.0f, 100.0f * (waits / ft));
    } else {
        stats.cpu_busy_pct_proxy = 0.0f;
        stats.gpu_busy_pct_proxy = 0.0f;
        stats.wait_frac_pct      = 0.0f;
    }
...
void LamureMeasurement::writeLogAndStop()
{
    ...
    // 6) Markdown (Front-Matter + sauber aus syn befüllt)
    {
        ensureParentDir(md_path);
        std::ofstream md(md_path, std::ios::out | std::ios::trunc);
        if (md) {
            ...
            md << "metrics:\n";
            md << "  acg_fps: " << syn.fps_avg << "\n";
            md << "  avg_frame_time_ms: "      << syn.avg_frame_time_ms << "\n";
            md << "  avg_cpu_main_ms: "        << syn.avg_cpu_main_ms   << "\n";
            md << "  avg_gpu_time_ms: "        << syn.avg_gpu_time_ms   << "\n";
            md << "  avg_wait_ms: "            << syn.avg_wait_ms       << "\n";
            md << "  p50_ms: " << syn.p50 << "\n";
            md << "  p95_ms: " << syn.p95 << "\n";
            md << "  p99_ms: " << syn.p99 << "\n";
            md << "  jitter_p99_minus_p50_ms: " << syn.jitter_J << "\n";
            md << "  stutter_count_gt_2x_median: " << syn.stutter_count_2xmedian << "\n";
            md << "  stutter_count_gt_33ms: "      << syn.stutter_count_33ms     << "\n";
            md << "  avg_rpts_points_per_ms: "     << syn.avg_rpts_points_per_ms << "\n";
            md << "  avg_coverage_c1: "            << syn.avg_covC1 << "\n";
            md << "  avg_screen_density_d: "       << syn.avg_covD  << "\n";
            md << "---\n\n";
            ...
            // Run Metadata / System Snapshot (wie bei dir, unverändert)
            md << "\n## Run Metadata\n";
...
#ifdef _WIN32
            {
                ...
                if (haveNvml) {
                    if (!gpuName.empty()) md << "- GPU (NVML):   " << gpuName << "\n";
                    md << "- GPU Util (NVML): " << util << " %\n";
            ...
            // Inline Summary ausschließlich aus syn
            md << "\n## Summary (inline)\n";
            md << "- Frames collected: " << syn.nFrames << "\n";
            md << "- Timeline blocks: "  << syn.timelineBlocks << "\n";
            md << "- Mode: "             << (mode_full ? "Full" : mode_light ? "Light" : "Off") << "\n";
            md << "- Sample N: "         << m_plugin->getSettings().measure_sample << "\n";
            md << "- Avg Frame Time (ms): " << syn.avg_frame_time_ms << "\n";
            md << "- Avg CPU Main (ms): "   << syn.avg_cpu_main_ms   << "\n";
            md << "- Avg GPU Time (ms): "   << syn.avg_gpu_time_ms   << "\n";
            md << "- Avg Wait (ms): "       << syn.avg_wait_ms       << "\n";
            md << "- Avg CPU Busy (%): "    << syn.avg_cpu_busy_pct  << "\n";
            md << "- Avg GPU Busy (%): "    << syn.avg_gpu_busy_pct  << "\n";
            md << "- Avg Wait Fraction (%): "<< syn.avg_wait_frac_pct << "\n\n";

            md << "\n## FPS & Primitive/Frame (derived)\n";
            md << "- Avg FPS: " << syn.fps_avg << "\n";
            md << "- FPS p50/p95/p99: " << syn.fps_p50 << " / " << syn.fps_p95 << " / " << syn.fps_p99 << "\n";
            md << "- Ø Primitive pro Frame: " << syn.prims_per_frame_avg << "\n";
            ...
            md << "- p95 (ms): " << syn.p95 << "\n";
            md << "- p99 (ms): " << syn.p99 << "\n";
            md << "- Jitter J (p99 - p50) (ms): " << syn.jitter_J << "\n";
            md << "- Stutter >2×Median (Count): " << syn.stutter_count_2xmedian << "\n";
            md << "- Stutter >33.33 ms (Count): " << syn.stutter_count_33ms      << "\n";

            md << "\n## Durchsatz/Effizienz\n";
            md << "- Avg Rpts (points/ms): "     << syn.avg_rpts_points_per_ms    << "\n";
            md << "- Avg avg_area_px_per_prim: " << syn.avg_area_px_per_prim      << "\n";

            md << "\n## Coverage (aus Estimates)\n";
            md << "- Avg Coverage C1: "          << syn.avg_covC1 << "\n";
            md << "- Avg Screen Density D: "     << syn.avg_covD  << "\n";

            md << "\n## Renderer Estimates (avg)\n";
            md << "- est_screen_px: "   << syn.est_screen_px   << "\n";
            md << "- est_sum_area_px: " << syn.est_sum_area_px << "\n";
            md << "- est_density: "     << syn.est_density     << "\n";
            md << "- est_coverage: "    << syn.est_coverage    << "\n";
            md << "- est_coverage_px: " << syn.est_coverage_px << "\n";
            md << "- est_overdraw: "    << syn.est_overdraw    << "\n";

            md << "\n## Renderer-Zeitmarken (Durchschnitt pro Frame)\n";
            md << "- draw_impl_ms: "    << syn.mark_draw_impl_ms    << "\n";
            md << "- pass1_ms: "        << syn.mark_pass1_ms        << "\n";
            md << "- pass2_ms: "        << syn.mark_pass2_ms        << "\n";
            md << "- pass3_ms: "        << syn.mark_pass3_ms        << "\n";
            md << "- dispatch_ms: "     << syn.mark_dispatch_ms     << "\n";
            md << "- context_bind_ms: " << syn.mark_context_bind_ms << "\n";
            md << "- estimates_ms: "    << syn.mark_estimates_ms    << "\n";
            md << "- singlepass_ms: "   << syn.mark_singlepass_ms   << "\n";
            ...
            // Policy Budgets + GPU Memory (static) wie bei dir …
            ...
            appendPreprocessBuildLogsMarkdown(md); // falls du das bereits hast
        } else {
...
bool LamureMeasurement::writeFramesCSV(const std::filesystem::path& frames_path,
    ...
    const std::unordered_map<unsigned,double>& gpuMsByFrame)
{
    ...
    if (lite) {
        out << "frame_number;frame_time_ms;rendering_traversals_ms;cpu_main_ms;gpu_time_ms;"
            "rendered_splats;rendered_nodes;est_density;est_coverage;est_overdraw;segment_index\n";
    } else {
        out << "frame_number;frame_rate;frame_time_ms;cpu_known_sum_ms;residual_time_ms;"
            ...
            "wait_ms;wait_frac_pct;boundness;rendered_splats;rendered_nodes;rendered_bounding_boxes;"
    ...
    for (auto& kv : statsByFrame) ids.insert(kv.first);
    for (auto& kv : gpuMsByFrame) ids.insert(kv.first);

    for (unsigned fid : ids) {
        FrameStats s{};
        if (auto it = statsByFrame.find(fid); it != statsByFrame.end()) s = it->second;
        else s.frame_number = fid;

        const double gms = (gpuMsByFrame.count(fid) ? gpuMsByFrame.at(fid) : 0.0);

        if (lite) {
            out << fid << ";" << s.frame_duration_ms << ";" << s.rendering_traversals_ms << ";"
                << s.cpu_main_ms << ";" << s.gpu_time_ms << ";" << s.rendered_splats << ";"
                << s.rendered_nodes << ";" << s.est_density << ";" << s.est_coverage << ";"
                << s.est_overdraw << ";" << s.segment_index << "\n";
            continue;
        }

        const double cpu_known =
        ...
        const double residual = std::max(0.0, s.frame_duration_ms - cpu_known);
        ...
        const float rpts = (gms > 1e-6) ? float(double(s.rendered_splats)/gms) : 0.0f;

        out << fid << ";" << s.frame_rate << ";" << s.frame_duration_ms << ";"
            << cpu_known << ";" << residual << ";"
            << s.rendering_traversals_ms << ";" << s.cpu_update_ms << ";" << s.cpu_cull_ms << ";"
            << s.cpu_draw_ms << ";" << s.gpu_time_ms << ";"
            << s.gpu_clock << ";" << s.gpu_mem_clock << ";" << s.gpu_util << ";" << s.gpu_pci << ";"
            << s.sync_time_ms << ";" << s.swap_time_ms << ";" << s.finish_ms << ";"
            << s.isect_ms << ";" << s.plugin_ms << ";" << s.opencover_ms << ";"
            << s.cpu_main_ms << ";" << s.cpu_busy_pct_proxy << ";" << s.gpu_busy_pct_proxy << ";"
            << s.wait_ms << ";" << s.wait_frac_pct << ";" << s.boundness << ";"
            << s.rendered_splats << ";" << s.rendered_nodes << ";" << s.rendered_bounding_boxes << ";"
            << s.est_screen_px << ";" << s.est_sum_area_px << ";" << s.est_density << ";"
            << s.est_coverage << ";" << s.est_coverage_px << ";" << s.est_overdraw << ";"
            << s.est_density_raw << ";" << s.est_coverage_raw << ";" << s.est_coverage_px_raw << ";"
            << s.est_overdraw_raw << ";"
            << s.position.x() << ";" << s.position.y() << ";" << s.position.z() << ";"
            << s.orientation.x() << ";" << s.orientation.y() << ";" << s.orientation.z() << ";" << s.orientation.w() << ";"
            << s.backoff_cull << ";" << s.backoff_draw << ";" << s.backoff_gpu << ";"
            << s.segment_index << ";" << rpts << ";" << double(s.avg_area_px_per_prim) << ";"
            << k_orient_used << ";"
            << s.mark_draw_impl_ms << ";" << s.mark_pass1_ms << ";" << s.mark_pass2_ms << ";"
            << s.mark_pass3_ms << ";" << s.mark_singlepass_ms << ";" << s.mark_dispatch_ms << ";"
            << s.mark_context_bind_ms << ";" << s.mark_estimates_ms << "\n";
    }
    ...
    const Synthesis& syn, bool mode_full, bool mode_light, bool hasTimeline)
{
    ...
    js << "{\n";
    js << "  \"generated_at_local\": \"" << iso8601_local_now() << "\",\n";
    js << "  \"generated_at_utc\": \""   << iso8601_utc_now()   << "\",\n";
    js << "  \"mode\": \"" << (mode_full ? "Full" : mode_light ? "Light" : "Off") << "\",\n";
    js << "  \"sample_n\": " << m_plugin->getSettings().measure_sample << ",\n";
    js << "  \"frames_collected\": " << syn.nFrames << ",\n";
    js << "  \"timeline_blocks\": "  << (hasTimeline ? syn.timelineBlocks : 0) << ",\n";
    js << "  \"metrics\": {\n";
    js << "    \"avg_frame_time_ms\": "  << syn.avg_frame_time_ms << ",\n";
    js << "    \"avg_cpu_main_ms\": "    << syn.avg_cpu_main_ms   << ",\n";
    js << "    \"avg_gpu_time_ms\": "    << syn.avg_gpu_time_ms   << ",\n";
    js << "    \"avg_wait_ms\": "        << syn.avg_wait_ms       << ",\n";
    js << "    \"p50_ms\": " << syn.p50 << ", \"p95_ms\": " << syn.p95 << ", \"p99_ms\": " << syn.p99 << ",\n";
    js << "    \"jitter_p99_minus_p50_ms\": " << syn.jitter_J << ",\n";
    js << "    \"stutter_count_gt_2x_median\": " << syn.stutter_count_2xmedian << ",\n";
    js << "    \"stutter_count_gt_33ms\": "      << syn.stutter_count_33ms     << ",\n";
    js << "    \"avg_rpts_points_per_ms\": "     << syn.avg_rpts_points_per_ms << ",\n";
    js << "    \"avg_coverage_c1\": "            << syn.avg_covC1 << ",\n";
    js << "    \"avg_screen_density_d\": "       << syn.avg_covD  << "\n";
    js << "  },\n";
    js << "  ,\"derived\": {\n";
    js << "    \"avg_fps\": "  << syn.fps_avg << ",\n";
    js << "    \"fps_p50\": "  << syn.fps_p50 << ",\n";
    js << "    \"fps_p95\": "  << syn.fps_p95 << ",\n";
    js << "    \"fps_p99\": "  << syn.fps_p99 << ",\n";
    js << "    \"prims_per_frame_avg\": " << syn.prims_per_frame_avg << ",\n";
    js << "    \"prims_per_frame_p50\": " << syn.prims_per_frame_p50 << ",\n";
    js << "    \"prims_per_frame_p95\": " << syn.prims_per_frame_p95 << ",\n";
    js << "    \"prims_per_frame_p99\": " << syn.prims_per_frame_p99 << "\n";
    js << "  }\n";
    js << "  \"gpu_memory_static\": {\n";
    js << "    \"primary_used_mb\": " << m_gpu_mem_used_mb_static  << ",\n";
    js << "    \"primary_total_mb\": " << m_gpu_mem_total_mb_static << ",\n";
    js << "    \"nvml_used_mb\": "    << m_gpu_mem_used_mb_nvml_static  << ",\n";
    js << "    \"nvml_total_mb\": "   << m_gpu_mem_total_mb_nvml_static << ",\n";
    js << "    \"glnvx_used_mb\": "   << m_gpu_mem_used_mb_gl_static    << ",\n";
    js << "    \"glnvx_total_mb\": "  << m_gpu_mem_total_mb_gl_static   << "\n";
    js << "  }\n";
    js << "}\n";
    js.flush();
    return true;
