...
#ifndef CO_REN_CUT_DATABASE_RECORD_H_
#define CO_REN_CUT_DATABASE_RECORD_H_
...
public:
    ...
    cut_database_record_lmr(const cut_database_record_lmr&) = delete;
    cut_database_record_lmr& operator=(const cut_database_record_lmr&) = delete;
    virtual             ~cut_database_record_lmr();

    void                set_cut(const view_t view_id, const model_t model_id, cut_lmr& cut_lmr);
    cut_lmr&                get_cut(const view_t view_id, const model_t model_id);
    ...
    void                set_camera(const view_t view_id, const camera_lmr& cam);
    ...
    void                receive_cameras(std::map<view_t, camera_lmr>& cameras);
...
private:
    ...
    std::vector<std::vector<cut_lmr>> front_a_cuts_;
    std::vector<std::vector<cut_lmr>> front_b_cuts_;
    ...
    std::map<view_t, camera_lmr> front_a_cameras_;
    std::map<view_t, camera_lmr> front_b_cameras_;
...
#endif // CO_REN_CUT_DATABASE_RECORD_H_
