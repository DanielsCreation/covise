...
#ifndef CO_REN_CUT_UPDATE_POOL_H_
#define CO_REN_CUT_UPDATE_POOL_H_
...
#include <cut.h>
#include <cut_update_index.h>
#include <cut_update_queue.h>
#include <gpu_cache.h>
#include <camera.h>
...
{
  public:
    cut_update_pool_lmr(const context_t context_id, const node_t upload_budget_in_nodes, const node_t render_budget_in_nodes, Data_Provenance_lmr const &data_provenance);
    cut_update_pool_lmr(const context_t context_id, const node_t upload_budget_in_nodes, const node_t render_budget_in_nodes);
    virtual ~cut_update_pool_lmr();
  ...
  protected:
    ...
    void split_node(const cut_update_index_lmr::action &item);
    void collapse_node(const cut_update_index_lmr::action &item);
    void cut_update_split_again(const cut_update_index_lmr::action &split_action);

    const bool is_all_nodes_in_cut(const model_t model_id, const std::vector<node_t> &node_ids, const std::set<node_t> &cut_lmr);
  ...
  private:
    ...
    cut_update_queue_lmr job_queue_;

    gpu_cache_lmr *gpu_cache_;
    cut_update_index_lmr *index_;

    std::vector<cut_database_record_lmr::slot_update_desc> transfer_list_;
    std::vector<std::vector<std::vector<cut_lmr::node_slot_aggregate>>> render_list_;
    ...
    cut_database_record_lmr::temporary_buffer current_gpu_buffer_;

    std::map<view_t, camera_lmr> user_cameras_;
...
// size_t                  out_of_core_budget_in_nodes_;
...
#ifdef LAMURE_CUT_UPDATE_ENABLE_PREFETCHING
    std::vector<cut_update_index_lmr::action> pending_prefetch_set_;
#endif
    ...
    semaphore_lmr master_semaphore_;
    bool master_dispatched_;
};
#endif // CO_REN_CUT_UPDATE_POOL_H_
