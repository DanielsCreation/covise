...
class camera_lmr
{
  public:
    ...
    camera_lmr() : cam_state_(CAM_STATE_GUA){};
    camera_lmr(const view_t view_id, float near_plane, scm::math::mat4f const &view, scm::math::mat4f const &proj);

    camera_lmr(const view_t view_id, float near_plane, float far_plane_value_, scm::math::mat4f const& view, scm::math::mat4f const& proj, scm::math::mat4f const& init_tb_mat, float distance, bool fast_travel = false, bool touch_screen_mode = false);

    camera_lmr(const view_t view_id, scm::math::mat4f const &init_tb_mat, float distance, bool fast_travel = false, bool touch_screen_mode = false);
    virtual ~camera_lmr();

    static camera_lmr* camera_lmr::instance();
    ...
    void sync_osg_cam(camera_lmr* cam_, osg::Camera* osg_cam_);
