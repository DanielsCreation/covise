...
cut_update_pool_lmr::cut_update_pool_lmr(const context_t context_id, const node_t upload_budget_in_nodes, const node_t render_budget_in_nodes, Data_Provenance_lmr const &data_provenance)
    : context_id_(context_id), locked_(false), num_threads_(LAMURE_CUT_UPDATE_NUM_CUT_UPDATE_THREADS), shutdown_(false), current_gpu_storage_A_(nullptr), current_gpu_storage_B_(nullptr),
      ...
      current_gpu_buffer_(cut_database_record_lmr::temporary_buffer::BUFFER_A), upload_budget_in_nodes_(upload_budget_in_nodes), render_budget_in_nodes_(render_budget_in_nodes),
      ...
      master_dispatched_(false)
{
    ...
    {
        threads_.push_back(std::thread(&cut_update_pool_lmr::run, this));
    }
}

cut_update_pool_lmr::cut_update_pool_lmr(const context_t context_id, const node_t upload_budget_in_nodes, const node_t render_budget_in_nodes)
    : context_id_(context_id), locked_(false), num_threads_(LAMURE_CUT_UPDATE_NUM_CUT_UPDATE_THREADS), shutdown_(false), current_gpu_storage_A_(nullptr), current_gpu_storage_B_(nullptr),
      current_gpu_storage_(nullptr), current_gpu_storage_A_provenance_(nullptr), current_gpu_storage_B_provenance_(nullptr), current_gpu_storage_provenance_(nullptr),
      current_gpu_buffer_(cut_database_record_lmr::temporary_buffer::BUFFER_A), upload_budget_in_nodes_(upload_budget_in_nodes), render_budget_in_nodes_(render_budget_in_nodes),
#ifdef LAMURE_CUT_UPDATE_ENABLE_MODEL_TIMEOUT
...
cut_update_pool_lmr::~cut_update_pool_lmr()
{
    {
...
void cut_update_pool_lmr::initialize(bool provenance)
{
    model_database_lmr *database = model_database_lmr::get_instance();
    policy_lmr *policy_lmr = policy_lmr::get_instance();

    assert(policy_lmr->render_budget_in_mb() > 0);
    assert(policy_lmr->out_of_core_budget_in_mb() > 0);

    index_ = new cut_update_index_lmr();
    index_->update_policy(0);
    gpu_cache_ = new gpu_cache_lmr(render_budget_in_nodes_);

    if (provenance) {
      ooc_cache_lmr *ooc_cache_lmr = ooc_cache_lmr::get_instance(_data_provenance);
    }
    else {
      ooc_cache_lmr *ooc_cache_lmr = ooc_cache_lmr::get_instance();
    }
    ...
    std::cout << "lamure: ooc-cache_lmr size (MB): " << policy_lmr->out_of_core_budget_in_mb() << std::endl;
#endif
}

void cut_update_pool_lmr::shutdown()
{
    std::lock_guard<std::mutex> lock(mutex_);
...
bool cut_update_pool_lmr::is_shutdown()
{
    std::lock_guard<std::mutex> lock(mutex_);
    return shutdown_;
}

const bool cut_update_pool_lmr::is_running()
{
    std::lock_guard<std::mutex> lock(mutex_);
    return master_dispatched_;
}

void cut_update_pool_lmr::dispatch_cut_update(char *current_gpu_storage_A, char *current_gpu_storage_B, char *current_gpu_storage_A_provenance, char *current_gpu_storage_B_provenance)
{
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    if(!master_dispatched_)
    {
        ...
        job_queue_.push_job(cut_update_queue_lmr::job(cut_update_queue_lmr::task_t::CUT_MASTER_TASK, invalid_view_t, invalid_model_t));

        semaphore_.signal(1);
    }
}

void cut_update_pool_lmr::run()
{
    while(true)
    {
        semaphore_.wait();
        ...
        // dequeue job
        cut_update_queue_lmr::job job = job_queue_.pop_front_job();

        if(job.task_ != cut_update_queue_lmr::task_t::CUT_INVALID_TASK)
        {
            switch(job.task_)
            {
            case cut_update_queue_lmr::task_t::CUT_MASTER_TASK:
                cut_master();
                break;

            case cut_update_queue_lmr::task_t::CUT_ANALYSIS_TASK:
                cut_analysis(job.view_id_, job.model_id_);
                break;

            case cut_update_queue_lmr::task_t::CUT_UPDATE_TASK:
                cut_update();
                break;
...
const bool cut_update_pool_lmr::prepare()
{
    cut_database_lmr *cut_database_lmr = cut_database_lmr::get_instance();

    cut_database_lmr->receive_cameras(context_id_, user_cameras_);
    cut_database_lmr->receive_height_divided_by_top_minus_bottoms(context_id_, height_divided_by_top_minus_bottoms_);
    cut_database_lmr->receive_transforms(context_id_, model_transforms_);
    cut_database_lmr->receive_thresholds(context_id_, model_thresholds_);
    ...
    cut_database_lmr->receive_rendered(context_id_, rendered_model_ids);
    ...
    if(!all_roots_resident)
    {
        ...
        ooc_cache_lmr->lock();
        ooc_cache_lmr->refresh();

        for(model_t model_id = 0; model_id < index_->num_models(); ++model_id)
        {
            if(!ooc_cache_lmr->is_node_resident(model_id, 0))
            {
                ooc_cache_lmr->register_node(model_id, 0, 100);
                all_roots_resident = false;
            }
        }

        ooc_cache_lmr->unlock();
        ...
        else
        {
            ...
            for(model_t model_id = 0; model_id < index_->num_models(); ++model_id)
            {
                ...
                for(view_t view_id = 0; view_id < index_->num_views(); ++view_id)
                {
                    if(index_->get_current_cut(view_id, model_id).empty())
                    {
                        assert(ooc_cache_lmr->is_node_resident(model_id, 0));
                        assert(gpu_cache_->is_node_resident(model_id, 0));

                        ooc_cache_lmr->aquire_node(context_id_, view_id, model_id, 0);
                        gpu_cache_->aquire_node(context_id_, view_id, model_id, 0);

                        index_->push_action(cut_update_index_lmr::action(cut_update_index_lmr::queue_t::KEEP, view_id, model_id, 0, 10000.f), false);
                    }
                }
            }
...
void cut_update_pool_lmr::cut_master()
{
    if(!prepare())
    {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    if(current_gpu_buffer_ == cut_database_record_lmr::temporary_buffer::BUFFER_A)
    {
        current_gpu_buffer_ = cut_database_record_lmr::temporary_buffer::BUFFER_B;
        current_gpu_storage_ = current_gpu_storage_B_;
        current_gpu_storage_provenance_ = current_gpu_storage_B_provenance_;
    }
    else
    {
        current_gpu_buffer_ = cut_database_record_lmr::temporary_buffer::BUFFER_A;
    ...
    while(true)
    {
        ...
        for(view_t view_id = 0; view_id < index_->num_views(); ++view_id)
        {
            for(model_t model_id = 0; model_id < index_->num_models(); ++model_id)
            {
                job_queue_.push_job(cut_update_queue_lmr::job(cut_update_queue_lmr::task_t::CUT_ANALYSIS_TASK, view_id, model_id));
            }
        }
        ...
        job_queue_.push_job(cut_update_queue_lmr::job(cut_update_queue_lmr::task_t::CUT_UPDATE_TASK, 0, 0));
        semaphore_.signal(1);
    ...
    {
        // model_database_lmr* database = model_database_lmr::get_instance();
        cut_database_lmr *cuts = cut_database_lmr::get_instance();
        ...
        for(model_t model_id = 0; model_id < index_->num_models(); ++model_id)
        {
            for(view_t view_id = 0; view_id < index_->num_views(); ++view_id)
            {
                cut_lmr cut_lmr(context_id_, view_id, model_id);
                cut_lmr.set_complete_set(render_list_[view_id][model_id]);

                cuts->set_cut(context_id_, view_id, model_id, cut_lmr);
            }
        }
    ...
    for(cut_it = old_cut.begin(); cut_it != old_cut.end(); ++cut_it)
    {
        ...
        if (!all_siblings_in_cut)
        {
            ...
            if (node_in_frustum && node_error > max_error_threshold && pvs->get_viewer_visibility(model_id, node_id))
            {
                ...
                {
                    index_->push_action(cut_update_index_lmr::action(cut_update_index_lmr::queue_t::KEEP, view_id, model_id, node_id, parent_error), false);
                }
                else
                {
                    index_->push_action(cut_update_index_lmr::action(cut_update_index_lmr::queue_t::MUST_SPLIT,view_id, model_id, node_id, node_error), false);
                }
            }
            else
            {
        ...
        else
        {
            ...
            if (no_sibling_in_frustum)
            {
                ...
                index_->push_action(cut_update_index_lmr::action(cut_update_index_lmr::queue_t::MUST_COLLAPSE, view_id, model_id, parent_id, parent_error), false);
#else
                index_->push_action(cut_update_index_lmr::action(cut_update_index_lmr::queue_t::COLLAPSE_ON_NEED, view_id, model_id, parent_id, parent_error), false);
#endif
            }
            ...
            {
                //the entire group of siblings is in the cut_lmr and visible
                ...
                for (const auto& sibling_id : siblings)
                {
                    ...
                    if (sibling_error > max_error_threshold && sibling_in_frustum && pvs->get_viewer_visibility(model_id, sibling_id))
                    {
                        ...
                        {
                            index_->push_action(cut_update_index_lmr::action(cut_update_index_lmr::queue_t::MUST_SPLIT, view_id, model_id, sibling_id, sibling_error), false);
                ...
                {
                    index_->push_action(cut_update_index_lmr::action(cut_update_index_lmr::queue_t::MAYBE_COLLAPSE, view_id, model_id, parent_id, parent_error), false);
                }
                else
                {
                    ...
                    for (uint32_t j = 0; j < fan_factor; ++j)
                    {
                        if (keep_sibling[j])
                        {
                            index_->push_action(cut_update_index_lmr::action(cut_update_index_lmr::queue_t::KEEP, view_id, model_id, siblings[j], parent_error), false);
                        }
                    }
                }
            }
...
void cut_update_pool_lmr::cut_update_split_again(const cut_update_index_lmr::action &split_action)
{
    ...
    for(const auto &candidate_id : candidates)
    {
        ...
        if(node_error > max_error_threshold)
        {
            ...
            if(!split)
            {
                index_->push_action(cut_update_index_lmr::action(cut_update_index_lmr::queue_t::KEEP, split_action.view_id_, split_action.model_id_, candidate_id, node_error), true);
            }
            else
            {
                index_->push_action(cut_update_index_lmr::action(cut_update_index_lmr::queue_t::MUST_SPLIT, split_action.view_id_, split_action.model_id_, candidate_id, node_error), true);
            }
        }
        else
        {
...
void cut_update_pool_lmr::cut_update()
{
    ooc_cache_lmr *ooc_cache_lmr = ooc_cache_lmr::get_instance(_data_provenance);
    ooc_cache_lmr->lock();
    ooc_cache_lmr->refresh();
    ...
    // cut_lmr update
    while(index_->num_actions(cut_update_index_lmr::queue_t::MUST_SPLIT) > 0)
    {
        cut_update_index_lmr::action must_split_action = index_->front_action(cut_update_index_lmr::queue_t::MUST_SPLIT);
        ...
        if(check_residency)
        {
            ...
            {
                if(!ooc_cache_lmr->is_node_resident(must_split_action.model_id_, child_id))
                {
            ...
            {
                index_->pop_front_action(cut_update_index_lmr::queue_t::MUST_SPLIT);

                for(const auto &child_id : child_ids)
                {
                    gpu_cache_->aquire_node(context_id_, must_split_action.view_id_, must_split_action.model_id_, child_id);
                    ooc_cache_lmr->aquire_node(context_id_, must_split_action.view_id_, must_split_action.model_id_, child_id);
                }
        ...
        bool all_children_fit_in_ooc_cache = ooc_cache_lmr->num_free_slots() >= fan_factor;
        ...
        {
            cut_update_index_lmr::action msa = index_->front_action(cut_update_index_lmr::queue_t::MUST_SPLIT);
            index_->pop_front_action(cut_update_index_lmr::queue_t::MUST_SPLIT);
        ...
        if(index_->num_actions(cut_update_index_lmr::queue_t::MUST_COLLAPSE) > 0)
        {
            cut_update_index_lmr::action collapse_action = index_->front_action(cut_update_index_lmr::queue_t::MUST_COLLAPSE);
            index_->pop_front_action(cut_update_index_lmr::queue_t::MUST_COLLAPSE);

            collapse_node(collapse_action);
            continue;
        }

        if(index_->num_actions(cut_update_index_lmr::queue_t::COLLAPSE_ON_NEED) > 0)
        {
            cut_update_index_lmr::action collapse_on_need_action = index_->front_action(cut_update_index_lmr::queue_t::COLLAPSE_ON_NEED);
            index_->pop_front_action(cut_update_index_lmr::queue_t::COLLAPSE_ON_NEED);

            collapse_node(collapse_on_need_action);
            continue;
        }

        if(index_->num_actions(cut_update_index_lmr::queue_t::MAYBE_COLLAPSE) > 0)
        {
            if(must_split_action.error_ > index_->back_action(cut_update_index_lmr::queue_t::MAYBE_COLLAPSE).error_)
            {
                cut_update_index_lmr::action collapse_action = index_->back_action(cut_update_index_lmr::queue_t::MAYBE_COLLAPSE);
                index_->Popback_action(cut_update_index_lmr::queue_t::MAYBE_COLLAPSE);
        ...
        if(index_->num_actions(cut_update_index_lmr::queue_t::KEEP) > 0)
        {
            cut_update_index_lmr::action keep_action = index_->back_action(cut_update_index_lmr::queue_t::KEEP);
            index_->Popback_action(cut_update_index_lmr::queue_t::KEEP);
                ...
                if(keep_action.node_id_ > 0 && keep_action_parent_id > 0)
                {
                    ...
                    if(is_all_nodes_in_cut(keep_action.model_id_, siblings, index_->get_previous_cut(keep_action.view_id_, keep_action.model_id_)))
                    {
                        ...
                        if(!singularity)
                        {
                            ...
                            index_->approve_action(cut_update_index_lmr::action(cut_update_index_lmr::queue_t::KEEP, keep_action.view_id_, keep_action.model_id_, keep_action_parent_id, keep_action.error_));
        ...
        if(index_->num_actions(cut_update_index_lmr::queue_t::MUST_SPLIT) > 1)
        { //> 1, prevent request from canceling itself
            cut_update_index_lmr::action split_action = index_->back_action(cut_update_index_lmr::queue_t::MUST_SPLIT);
            index_->Popback_action(cut_update_index_lmr::queue_t::MUST_SPLIT);
            ...
            if(must_split_action.error_ > split_action.error_)
            {
                ...
                {
                    // only if siblings are also in cut_lmr -- why does it make sense to check this here?
                    ...
                    // to cut_lmr down memory usage in the end.
                    std::vector<node_t> siblings;
                    ...
                    if(is_all_nodes_in_cut(split_action.model_id_, siblings, index_->get_previous_cut(split_action.view_id_, split_action.model_id_)))
                    {
                        ...
                        if(!singularity)
                        {
                            ...
                            index_->push_action(
                                cut_update_index_lmr::action(cut_update_index_lmr::queue_t::KEEP, split_action.view_id_, split_action.model_id_, split_action.node_id_, replacement_node_error * 2.75f), true);
    ...
    while(index_->num_actions(cut_update_index_lmr::queue_t::MUST_COLLAPSE) > 0)
    {
        cut_update_index_lmr::action collapse_action = index_->front_action(cut_update_index_lmr::queue_t::MUST_COLLAPSE);
        index_->pop_front_action(cut_update_index_lmr::queue_t::MUST_COLLAPSE);
        collapse_node(collapse_action);
    ...
    while(index_->num_actions(cut_update_index_lmr::queue_t::COLLAPSE_ON_NEED) > 0)
    {
        cut_update_index_lmr::action collapse_on_need_action = index_->front_action(cut_update_index_lmr::queue_t::COLLAPSE_ON_NEED);
        index_->pop_front_action(cut_update_index_lmr::queue_t::COLLAPSE_ON_NEED);
    ...
    while(index_->num_actions(cut_update_index_lmr::queue_t::MAYBE_COLLAPSE) > 0)
    {
        cut_update_index_lmr::action maybe_collapse_action = index_->front_action(cut_update_index_lmr::queue_t::MAYBE_COLLAPSE);
        index_->pop_front_action(cut_update_index_lmr::queue_t::MAYBE_COLLAPSE);
        index_->reject_action(maybe_collapse_action);
    }

    // approve all keep-actions
    while(index_->num_actions(cut_update_index_lmr::queue_t::KEEP) > 0)
    {
        cut_update_index_lmr::action keep_action = index_->front_action(cut_update_index_lmr::queue_t::KEEP);
        index_->pop_front_action(cut_update_index_lmr::queue_t::KEEP);
        index_->approve_action(keep_action);
    }

    assert(index_->num_actions(cut_update_index_lmr::queue_t::KEEP) == 0);
    assert(index_->num_actions(cut_update_index_lmr::queue_t::MUST_SPLIT) == 0);
    assert(index_->num_actions(cut_update_index_lmr::queue_t::MUST_COLLAPSE) == 0);
    assert(index_->num_actions(cut_update_index_lmr::queue_t::COLLAPSE_ON_NEED) == 0);
    assert(index_->num_actions(cut_update_index_lmr::queue_t::MAYBE_COLLAPSE) == 0);
...
void cut_update_pool_lmr::compile_render_list()
{
    render_list_.clear();
    ...
    {
        std::vector<std::vector<cut_lmr::node_slot_aggregate>> view_render_lists;

        for(model_t model_id = 0; model_id < index_->num_models(); ++model_id)
        {
            std::vector<cut_lmr::node_slot_aggregate> model_render_lists;

            const std::set<node_t> &current_cut = index_->get_current_cut(view_id, model_id);

            for(const auto &node_id : current_cut)
            {
                model_render_lists.push_back(cut_lmr::node_slot_aggregate(node_id, gpu_cache_->slot_id(model_id, node_id)));
            }
...
void cut_update_pool_lmr::compile_transfer_list()
{
    model_database_lmr *database = model_database_lmr::get_instance();
    ooc_cache_lmr *ooc_cache_lmr = ooc_cache_lmr::get_instance(_data_provenance);
    ...
    for(model_t model_id = 0; model_id < index_->num_models(); ++model_id)
    {
        for(const auto &node_id : transfer_list[model_id])
        {
            ...
            char *node_data = ooc_cache_lmr->node_data(model_id, node_id);
            char *node_data_provenance = ooc_cache_lmr->node_data_provenance(model_id, node_id);
            ...
            transfer_list_.push_back(cut_database_record_lmr::slot_update_desc(slot_count, slot_id));
...
void cut_update_pool_lmr::split_node(const cut_update_index_lmr::action &action)
{
    // hack: split until depth-1
    const auto bvh = model_database_lmr::get_instance()->get_model(action.model_id_)->get_bvh();
    ...
    for(const auto &child_id : child_ids)
    {
        if(!ooc_cache_lmr->is_node_resident(action.model_id_, child_id))
        {
            if(all_children_fit_in_ooc_cache)
            {
                // load child from harddisk
                if(ooc_cache_lmr->num_free_slots() > 0)
                {
                    ooc_cache_lmr->register_node(action.model_id_, child_id, (int32_t)action.error_);
                }
            }
    ...
    if(all_children_available)
    {
        for(const auto &child_id : child_ids)
        {
            ...
            ooc_cache_lmr->aquire_node(context_id_, action.view_id_, action.model_id_, child_id);
        }
...
void cut_update_pool_lmr::collapse_node(const cut_update_index_lmr::action &action)
{
    ...
    for(const auto &child_id : child_ids)
    {
        ...
        ooc_cache_lmr->release_node(context_id_, action.view_id_, action.model_id_, child_id);
    }

    index_->approve_action(action);
}

const bool cut_update_pool_lmr::is_all_nodes_in_cut(const model_t model_id, const std::vector<node_t> &node_ids, const std::set<node_t> &cut_lmr)
{
    for(node_t i = 0; i < node_ids.size(); ++i)
    {
        ...
        if(cut_lmr.find(node_id) == cut_lmr.end())
            return false;
    }
...
const bool cut_update_pool_lmr::is_node_in_frustum(const view_t view_id, const model_t model_id, const node_t node_id, const scm::gl::frustum &frustum)
{
    model_database_lmr *database = model_database_lmr::get_instance();
...
const bool cut_update_pool_lmr::is_no_node_in_frustum(const view_t view_id, const model_t model_id, const std::vector<node_t> &node_ids, const scm::gl::frustum &frustum)
{
...
const float cut_update_pool_lmr::calculate_node_error(const view_t view_id, const model_t model_id, const node_t node_id)
{
    model_database_lmr *database = model_database_lmr::get_instance();
