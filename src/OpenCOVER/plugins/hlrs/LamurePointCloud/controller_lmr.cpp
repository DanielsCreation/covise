...
std::mutex controller_lmr::mutex_;
bool controller_lmr::is_instanced_ = false;
controller_lmr *controller_lmr::single_ = nullptr;

controller_lmr::controller_lmr() : num_contexts_registered_(0), num_models_registered_(0), ms_since_last_node_upload_(0), latest_timestamp_(std::chrono::system_clock::now()) {}

controller_lmr::~controller_lmr()
{
    ...
    {
        cut_update_pool_lmr *pool = cut_update_pool_it.second;
        if(pool != nullptr)
        {
            delete pool;
            pool = nullptr;
    ...
    {
        gpu_context_lmr *ctx = gpu_context_it.second;
        if(ctx != nullptr)
        {
            delete ctx;
            ctx = nullptr;
...
controller_lmr *controller_lmr::get_instance()
{
    if(!is_instanced_)
    {
        std::lock_guard<std::mutex> lock(mutex_);
...
const context_t controller_lmr::num_contexts_registered()
{
    std::lock_guard<std::mutex> lock(mutex_);
...
void controller_lmr::signal_system_reset()
{
    std::lock_guard<std::mutex> lock(mutex_);

    policy_lmr *policy_lmr = policy_lmr::get_instance();
    policy_lmr->set_reset_system(true);
...
const bool controller_lmr::is_system_reset_signaled(const context_t context_id)
{
    std::lock_guard<std::mutex> lock(mutex_);
...
void controller_lmr::reset_system()
{
    policy_lmr *policy_lmr = policy_lmr::get_instance();

    if(policy_lmr->reset_system())
    {
        model_database_lmr *database = model_database_lmr::get_instance();
        cut_database_lmr *cuts = cut_database_lmr::get_instance();
        controller_lmr *controller = controller_lmr::get_instance();
        ...
        if(policy_lmr->reset_system())
        {
            ...
            for(auto &gpu_context_it : gpu_contexts_)
            {
                gpu_context_lmr *context = gpu_context_it.second;
                if(context != nullptr)
                {
                    delete context;
                    context = nullptr;
            ...
            policy_lmr->set_reset_system(false);
        }
    }
}

void controller_lmr::reset_system(Data_Provenance_lmr const &data_provenance)
{
    policy_lmr *policy_lmr = policy_lmr::get_instance();
...
context_t controller_lmr::deduce_context_id(const gua_context_desc_t context_desc)
{
    ...
    {
        {
            ...
            gpu_contexts_.insert(std::make_pair(num_contexts_registered_, new gpu_context_lmr(num_contexts_registered_)));
...
view_t controller_lmr::deduce_view_id(const gua_context_desc_t context_desc, const gua_view_desc_t view_desc)
{
...
model_t controller_lmr::deduce_model_id(const gua_model_desc_t &model_desc)
{
...
const bool controller_lmr::is_cut_update_in_progress(const context_t context_id, Data_Provenance_lmr const &data_provenance)
{
    ...
    else
    {
        ...
        cut_update_pools_[context_id] = new cut_update_pool_lmr(context_id, ctx->upload_budget_in_nodes(), ctx->render_budget_in_nodes(), data_provenance);
...
const bool controller_lmr::is_cut_update_in_progress(const context_t context_id)
{
    auto gpu_context_it = gpu_contexts_.find(context_id);
    ...
    else
    {
        ...
        cut_update_pools_[context_id] = new cut_update_pool_lmr(context_id, ctx->upload_budget_in_nodes(), ctx->render_budget_in_nodes());
...
void controller_lmr::dispatch(const context_t context_id, boost::shared_ptr<LamureDevice> device, Data_Provenance_lmr const &data_provenance)
{
    auto gpu_context_it = gpu_contexts_.find(context_id);
    ...
    if(cut_update_it != cut_update_pools_.end())
    {
        ...
        {
            cut_database_record_lmr::temporary_buffer current = cuts->get_buffer(context_id);
...
void controller_lmr::dispatch(const context_t context_id, boost::shared_ptr<LamureDevice> device)
{
    auto gpu_context_it = gpu_contexts_.find(context_id);
...
const bool controller_lmr::is_model_present(const gua_model_desc_t model_desc) { return model_map_.find(model_desc) != model_map_.end(); }

scm::gl::buffer_ptr controller_lmr::get_context_buffer(const context_t context_id, lmr_device device)
{
    auto gpu_context_it = gpu_contexts_.find(context_id);
...
scm::gl::buffer_ptr controller_lmr::get_context_buffer(const context_t context_id, lmr_device device, Data_Provenance_lmr const &data_provenance)
{
    auto gpu_context_it = gpu_contexts_.find(context_id);
...
scm::gl::vertex_array_ptr controller_lmr::get_context_memory(const context_t context_id, bvh::primitive_type type, lmr_device device)
{
    auto gpu_context_it = gpu_contexts_.find(context_id);
...
scm::gl::vertex_array_ptr controller_lmr::get_context_memory(const context_t context_id, bvh::primitive_type type, lmr_device device, Data_Provenance_lmr const &data_provenance)
{
    auto gpu_context_it = gpu_contexts_.find(context_id);
