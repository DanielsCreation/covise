...
std::mutex model_database_lmr::mutex_;
bool model_database_lmr::is_instanced_ = false;
model_database_lmr* model_database_lmr::single_ = nullptr;
bool model_database_lmr::contains_only_compressed_data_ = true;
bool model_database_lmr::contains_trimesh_ = false;
...
~model_database_lmr() {
    ...
    for (const auto& model_it : datasets_) {
        dataset_lmr* model = model_it.second;
        if (model != nullptr) {
            delete model;
            model = nullptr;
        }
...
model_database_lmr* model_database_lmr::
get_instance() {
    if (!is_instanced_) {
        std::lock_guard<std::mutex> lock(mutex_);

        if (!is_instanced_) { //double-checked locking
            single_ = new model_database_lmr();
            is_instanced_ = true;
        }
...
const model_t model_database_lmr::
add_model(const std::string& filepath, const std::string& model_key) {

    if (controller_lmr::get_instance()->is_model_present(model_key)) {
        return controller_lmr::get_instance()->deduce_model_id(model_key);
    }

    dataset_lmr* model = new dataset_lmr(filepath);

    if (model->is_loaded()) {
        const bvh* bvh = model->get_bvh();

        if( bvh::primitive_type::POINTCLOUD_QZ != bvh->get_primitive() ) {
            model_database_lmr::contains_only_compressed_data_ = false;
        }

        if( bvh::primitive_type::TRIMESH == bvh->get_primitive() ) {
            model_database_lmr::contains_trimesh_ = true;
        }
...

        {
            ...
            model_id = controller_lmr::get_instance()->deduce_model_id(model_key);
            ...
            controller_lmr::get_instance()->signal_system_reset();
        }
        ...
        throw std::runtime_error(
            "lamure: model_database_lmr::Model was not loaded");
    }
...
dataset_lmr* model_database_lmr::
get_model(const model_t model_id) {
    if (datasets_.find(model_id) != datasets_.end()) {
        return datasets_[model_id];
    }
    throw std::runtime_error(
        "lamure: model_database_lmr::Model was not found:" + std::to_string(model_id));
    return nullptr;
}

const size_t model_database_lmr::
get_primitive_size(const bvh::primitive_type type) const {
    switch (type) {
        case bvh::primitive_type::POINTCLOUD:
            return sizeof(dataset_lmr::serialized_surfel);
        case bvh::primitive_type::POINTCLOUD_QZ:
            return sizeof(dataset_lmr::serialized_surfel_qz);
        case bvh::primitive_type::TRIMESH:
            return sizeof(dataset_lmr::serialized_vertex);
        default: break;
    }
    throw std::runtime_error(
        "lamure: model_database_lmr::Invalid primitive type has size 0");
    return 0;
}
...
get_slot_size() const {
    ...
    if( model_database_lmr::contains_only_compressed_data_ ) {
      return primitives_per_node_ * sizeof(dataset_lmr::serialized_surfel_qz);
    }
    else {
      if (model_database_lmr::contains_trimesh_) {
        return primitives_per_node_ * sizeof(dataset_lmr::serialized_vertex);
      }
      else {
        return primitives_per_node_ * sizeof(dataset_lmr::serialized_surfel);    
      }
    }
    return 0;
